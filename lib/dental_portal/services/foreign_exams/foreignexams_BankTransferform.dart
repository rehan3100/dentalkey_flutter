...
class FEBankTransferForm extends StatefulWidget {
  final String userId;
  ...
  @override
  _FEBankTransferFormState createState() =>
      _FEBankTransferFormState(accessToken: accessToken);
}

class _FEBankTransferFormState extends State<FEBankTransferForm> {
  File? _image;
  ...
  final String accessToken;
  _FEBankTransferFormState({required this.accessToken});
  ...
  Future<void> fetchBankTransferOptions() async {
    try {
      final Uri apiUrl = Uri.parse(
          'https://dental-key-738b90a4d87a.herokuapp.com/quizzer/bank_transfer_options/');
  ...
  Future<void> submitForm() async {
    final Uri apiUrl = Uri.parse(
        'https://dental-key-738b90a4d87a.herokuapp.com/quizzer/bank_payment/');
    ...
    request.fields['RequestQuiz'] = widget.orderId;
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        final updateOrderStatusUrl = Uri.parse(
            'https://dental-key-738b90a4d87a.herokuapp.com/quizzer/api/v1/orders/${widget.orderId}/update_status/');
        final updateOrderResponse = await http.post(
          updateOrderStatusUrl,
          headers: {'Authorization': 'Bearer ${widget.accessToken}'},
          body: {'request_order_status': 'PD'},
        );
        ...
        if (updateOrderResponse.statusCode == 200) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => ForeignOrders(accessToken: accessToken)),
          );
        } else {
          print(
