...
import 'package:dental_key/dental_portal/services/career_pathways/CP_BankTransferform.dart';
...
class CPDentalMyOrders extends StatefulWidget {
  final String accessToken;

  const CPDentalMyOrders({Key? key, required this.accessToken})
      : super(key: key);

  @override
  _CPDentalMyOrdersState createState() =>
      _CPDentalMyOrdersState(accessToken: accessToken);
}

class _CPDentalMyOrdersState extends State<CPDentalMyOrders> {
  List<dynamic> orders = [];
  bool isLoading = true;
  ...
  Future<void> fetchOrders() async {
    final String apiUrl =
        'https://dental-key-738b90a4d87a.herokuapp.com/career_pathways/user_orders/';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        fetchedOrders.sort((a, b) => DateTime.parse(b['package_order_date'])
            .compareTo(DateTime.parse(a['package_order_date'])));
  ...
  final String accessToken;
  _CPDentalMyOrdersState({required this.accessToken});
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text('My Career Pathway Orders'),
        ),
                                            ...
                                            ListTile(
                                              title: Column(
                                                ...
                                                children: [
                                                  ...
                                                  Text(
                                                      'Country: ${orders[index]['country']}'),
                                                  Text('Packages included:'),
                                                  Column(
                                                    crossAxisAlignment:
                                                        CrossAxisAlignment
                                                            .start,
                                                    children: List.generate(
                                                      orders[index][
                                                              'selected_packages_names']
                                                          .length,
                                                      (packageIndex) => Text(
                                                          '- ${orders[index]['selected_packages_names'][packageIndex]}'),
                                                    ),
                                                  ),
                                                  Text(
                                                      'Date & Time: ${_formatDateTime(orders[index]['package_order_date'])}'),
                                                  Text(
                                                      'Total Price: GBP ${orders[index]['package_total_price']}'),
                                                  Text(
                                                      'Chosen Payment Method: ${_formatPaymentMethod(orders[index]['package_payment_method'])}'),
                                                  Text(
                                                      'Current Order Status: ${orders[index]['package_status']}'),
                                                ],
                                              ),
                                            ),
                                            Padding(
                                                ...
                                                child: ElevatedButton(
                                                  onPressed: orders[index][
                                                      ...
                                                      : () {
                                                          ...
                                                          String orderprice =
                                                              orders[index][
                                                                  'package_order_total_price'];
                                                          print(
                                                              'Order is Awaiting Payment: ${orders[index]['package_order_status']}');
