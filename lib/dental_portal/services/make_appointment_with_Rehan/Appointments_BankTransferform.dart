...
class AppointmentsBankTransferForm extends StatefulWidget {
  final String userId;
  ...
  @override
  _AppointmentsBankTransferFormState createState() =>
      _AppointmentsBankTransferFormState(accessToken: accessToken);
}

class _AppointmentsBankTransferFormState
    extends State<AppointmentsBankTransferForm> {
  File? _image;
  ...
  final String accessToken;
  _AppointmentsBankTransferFormState({required this.accessToken});
  ...
  Future<void> fetchBankTransferOptions() async {
    try {
      final Uri apiUrl = Uri.parse(
          'https://dental-key-738b90a4d87a.herokuapp.com/appointments_with_dr_rehan/bank_transfer_options/');
  ...
  Future<void> submitForm() async {
    final Uri apiUrl = Uri.parse(
        'https://dental-key-738b90a4d87a.herokuapp.com/appointments_with_dr_rehan/bank_payment/');
    ...
    request.fields['AppointmentsRequests'] = widget.orderId;
    request.fields['request_total_price'] = widget.orderprice;
    ...
    if (_image != null) {
      ...
      request.files.add(await http.MultipartFile.fromPath(
        'payment_screenshot',
        _image!.path,
        filename: 'payment_screenshot.jpg', // Adjust filename if necessary
        contentType:
            MediaType('image', 'jpg'), // Adjust contentType if necessary
      ));
    } else {
      print('No image selected.');
      setState(() {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        final updateOrderStatusUrl = Uri.parse(
            'https://dental-key-738b90a4d87a.herokuapp.com/appointments_with_dr_rehan/api/v1/orders/${widget.orderId}/update_status/');
