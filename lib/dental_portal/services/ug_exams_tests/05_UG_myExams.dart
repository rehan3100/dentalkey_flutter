...
import 'package:dental_key/dental_portal/services/ug_exams_tests/06_UG_quiz_page.dart';

class UGMyExams extends StatefulWidget {
  final String accessToken;
  final String email;
  const UGMyExams({Key? key, required this.accessToken, required this.email})
      : super(key: key);

  @override
  _UGMyExamsState createState() => _UGMyExamsState(accessToken: accessToken);
}

class _UGMyExamsState extends State<UGMyExams> {
  bool isLoading = true;
  final String accessToken;
  List<UGQuiz> quizzes = [];

  _UGMyExamsState({required this.accessToken});

  Future<void> _fetchQuizzes() async {
    try {
      final response = await http.get(
        Uri.parse(
            'https://dental-key-738b90a4d87a.herokuapp.com/quizzer/requested-quizzes/undergraduate/'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            quizzes =
                responseData.map((data) => UGQuiz.fromJson(data)).toList();
            isLoading = false;
          });
        }
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('My Undergraduate Tests'),
      ),
